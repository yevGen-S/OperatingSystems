...
int main(void) {
    ...
    if ((shmid = shmget(key, sizeof(Message), PERM | IPC_CREAT)) == -1) {
    ...
    if (!(msgptr = (Message*)shmat(shmid, 0, 0)) ) {
    ...
    if ((semid = semget(key, 2, PERM | IPC_CREAT)) == -1) {
    ...
    //ожидание начала работы клиента
        ...
        if (semop(semid, &proc_wait[0], 1) == -1) {
    ...
    //ожидание завершения работы клиента с разделяемой памятью
        ...
        printf("Server: Read request from client\n%s", msgptr->buff);


        const size_t fn_size = MAXBUFF + 50;


        char* name = (char*)calloc(strlen(msgptr->buff) , sizeof(char));
        for (int i = 0; i < strlen(msgptr->buff) - 1; i ++) {
            name[i] = msgptr->buff[i];
        }


        printf("filename : %s\n\n", name);
        printf ("Len of msgptr->buff %lu\n", strlen(name));


        FILE* file = fopen(name, "r+");
        if (file == NULL) {
            sprintf(msgptr->buff, "Error: can't open the file by the abs path: %s", name);
        }
        else {
            char file_content[MAXBUFF - 20];
            fgets(file_content, MAXBUFF - 20, file);
            sprintf(msgptr->buff, "File content: %s\n\n", file_content);
        }
